...
func drawField(field map[Point]rune) {
...
func main() {
	...
	y, x := 0, 0
	for value := range vm.output {
		if value == '\n' {
			y++
			x = 0
			continue
		}
		field[Point{x, y}] = rune(value)
		x++
	}
	...
	for point, kind := range field {
		...
		for _, delta := range deltas {
			neighbor := Point{point.x + delta.x, point.y + delta.y}

			if nKind, ok := field[neighbor]; ok && nKind == '#' {
				validNeighbors++
			}
		}
	...
	fmt.Println("Part 1 solution is", result)
}
